...
LABEL="${1:?label}"
OUT="$REPAIR/$LABEL"
SESSION="trail-b-repair-$LABEL"
JUMPER="$REPAIR/jumper.py"
...
export MR_CRABS_REDUCE_MOTION="${MR_CRABS_REDUCE_MOTION:-0}"
echo "$MR_CRABS_REDUCE_MOTION" >"$OUT/reduce_motion_env.txt"
...
stat -f %m "$BIN" >"$OUT/binary_mtime.txt"
...
CMD="python3 $(printf '%q' "$JUMPER")"
"$CUA" clipboard_write "{\"text\":$(python3 -c 'import json,sys; print(json.dumps(sys.argv[1]))' "$CMD")}" >"$OUT/clip.json"
...
# Wait until AAAAA appears, then fire the CSI-left jump and burst-capture.
python3 - "$OUT" "$WIN_ID" <<'PY'
import subprocess, sys, time
from pathlib import Path
out = Path(sys.argv[1])
win = sys.argv[2]
pre = out / "pre_jump.png"
for i in range(40):
    time.sleep(0.15)
    r = subprocess.run(
        ["screencapture", "-x", "-o", "-S", "-l", str(win), str(pre)],
        capture_output=True, timeout=5,
    )
    if r.returncode == 0 and pre.exists() and pre.stat().st_size > 0:
        # crude: continue until file grows after first write
        if i >= 8:
            break
(out / "TRIGGER.flag").write_text("go\n")
frames = out / "frames"
...
if ! python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); sys.exit(0 if "old_box" in d else 1)' "$REPAIR/geometry.json"; then
  python3 "$REPAIR/fixed_geometry.py" discover "$OUT/pre_jump.png"
fi
python3 "$REPAIR/fixed_geometry.py" analyze "$OUT"
