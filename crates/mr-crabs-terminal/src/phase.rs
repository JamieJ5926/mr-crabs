//! Phase timing probes for `mr-crabs-terminal`.
//! Compiled away without `phase-timing` feature. With feature, inactive
//! unless `MR_CRABS_PHASE` / `MR_CRABS_PHASE_TIMING` is set: no Instant,
//! alloc, lock, or per-cell work.
...
mod tests {
    ...
    fn public_api_does_not_panic() {
        let _ = enabled();
        record("terminal::test", 42);
        let _ = snapshot();
        let _ = snapshot_map();
        let prev = snapshot_map();
        let _ = delta_since(&prev);
        flush_sidecar();
        let _guard = Guard::new("terminal::test");
    }

    #[test]
    fn guard_drop_without_record_is_noop() {
        {
            let _guard = Guard::new("terminal::guard_noop");
        }
    }
    ...
    mod enabled_helpers {
        ...
        fn record_into_aggregates_counts_and_nanos() {
            ...
            record_into(&reg, "terminal::feed", 10);
            record_into(&reg, "terminal::feed", 20);
            record_into(&reg, "terminal::drain", 5);
            let map = snapshot_map_from(&reg);
            assert_eq!(map.get("terminal::feed"), Some(&(2, 30)));
            assert_eq!(map.get("terminal::drain"), Some(&(1, 5)));
        }
