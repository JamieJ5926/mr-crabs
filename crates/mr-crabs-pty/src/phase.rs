//! Phase timing probes for `mr-crabs-pty`.
//!
//! With the `phase-timing` feature disabled this module compiles away and
//! provides zero-cost no-ops. With the feature enabled but the explicit
//! environment switch absent (`MR_CRABS_PHASE` / `MR_CRABS_PHASE_TIMING` not
//! set) probes do no `Instant` calls, allocations, locks, or per-cell work.
...
mod tests {
    ...
    fn public_api_does_not_panic() {
        ...
        flush_sidecar();
        let _guard = Guard::new("pty::test");
    }

    #[test]
    fn guard_drop_without_record_is_noop() {
        {
            let _guard = Guard::new("pty::guard_noop");
        }
    }
    ...
    mod enabled_helpers {
        ...
        fn record_into_aggregates_counts_and_nanos() {
            ...
            record_into(&reg, "pty::feed", 10);
            record_into(&reg, "pty::feed", 20);
            record_into(&reg, "pty::drain", 5);
            let map = snapshot_map_from(&reg);
            assert_eq!(map.get("pty::feed"), Some(&(2, 30)));
            assert_eq!(map.get("pty::drain"), Some(&(1, 5)));
        }
