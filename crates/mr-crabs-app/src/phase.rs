//! Additive phase probes for `mr-crabs-app` (pane/frame/cache).
//! Default-off compile-time gate + runtime env switch semantics mirror pty/terminal.
...
mod tests {
    ...
    fn public_api_does_not_panic() {
        ...
        flush_sidecar();
        let _guard = Guard::new("app::test");
    }

    #[test]
    fn guard_drop_without_record_is_noop() {
        {
            let _guard = Guard::new("app::guard_noop");
        }
    }
    ...
    mod enabled_helpers {
        ...
        fn record_into_aggregates_counts_and_nanos() {
            ...
            record_into(&reg, "app::feed", 10);
            record_into(&reg, "app::feed", 20);
            record_into(&reg, "app::drain", 5);
            let map = snapshot_map_from(&reg);
            assert_eq!(map.get("app::feed"), Some(&(2, 30)));
            assert_eq!(map.get("app::drain"), Some(&(1, 5)));
        }
