...
pub fn run_animated_fetch_and_exit() -> ! {
    ...
    match captured {
        ...
        Some(original) => {
            ...
            if !fits_terminal(&layout, &original, rows, cols) {
                let _ = std::io::stdout().write_all(original.as_bytes());
                let _ = std::io::stdout().flush();
                std::process::exit(0);
            }
...
mod tests {
    ...
    fn sample_output() -> String {
        ...
        s.push_str(":_________:      CPU: Apple M4 Pro\n");
        s.push_str(" :_________`-;   Screen: 3440x1440\n");
        s.push_str("  `.__.-.__.'    RAM: 32 GB\n");
        s
    }

    #[test]
    fn layout_splitting_locates_dashed_separator() {
        let out = sample_output();
        let layout = parse_fetch_layout(&out).expect("layout");
        assert_eq!(layout.logo_width, 17);
        assert_eq!(layout.lines.len(), 7);
        assert_eq!(layout.lines[1].logo, "    __ :'__      ");
        assert_eq!(layout.lines[1].info, "------------------------------");
        assert_eq!(layout.lines[2].info, "OS: Darwin (aarch64)");
        assert_eq!(layout.lines[2].logo, " .'`__`-'__``.   ");
    }

    #[test]
    fn logo_only_colorization() {
        let out = sample_output();
        let layout = parse_fetch_layout(&out).expect("layout");
        let frame = frame_bytes(&layout, 0);
        let text = String::from_utf8(frame).expect("utf8");
        for line in text.lines() {
            if let Some(idx) = line.find("OS:") {
                let suffix = &line[idx..];
                assert!(
                    !suffix.contains("\x1b["),
                    "info column must remain unchanged"
                );
            }
            if line.contains("CPU:") {
                let idx = line.find("CPU:").expect("cpu");
                let suffix = &line[idx..];
                assert!(!suffix.contains("\x1b["), "info must not be colorized");
            }
        }
        assert!(text.contains("\x1b[38;2;"), "frame must contain truecolor");
        let first_logo_part = text.lines().next().unwrap();
        assert!(
            first_logo_part.contains("\x1b[38;2;"),
            "non-space glyphs get color"
        );
        assert!(!text.contains("\x1b[38;2;") || text.contains("\x1b[0m"));
    }

    #[test]
    fn fixed_height_inline_rewrites() {
        let out = sample_output();
        let layout = parse_fetch_layout(&out).expect("layout");
        let bytes = inline_animation_bytes(&layout, &out);
        let text = String::from_utf8_lossy(&bytes);
        let height = layout.lines.len();
        let esc = format!("\x1b[{height}A");
        let count = text.matches(&esc).count();
        assert_eq!(count, FRAME_COUNT, "must rewrite same fixed-height block");
        assert!(
            !text.contains("\x1b[?1049"),
            "must not use alternate screen"
        );
        assert!(!text.contains("\x1b[?25l"), "must not hide cursor");
        assert!(!text.contains("\x1b[?25h"), "must not hide cursor");
    }

    #[test]
    fn final_exact_static_output() {
        let out = sample_output();
        let layout = parse_fetch_layout(&out).expect("layout");
        let bytes = inline_animation_bytes(&layout, &out);
        assert!(
    ...
    fn startup_command_propagation_in_pane() {
        ...
        let cfg = PtySpawnConfig {
            ...
            startup_command: Some(mr_crabs_config::DEFAULT_STARTUP_FETCH_COMMAND.to_string()),
        };
        assert_eq!(
