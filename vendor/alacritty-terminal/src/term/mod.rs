...
impl<T: EventListener> Term<T> {
    ...
    fn input_ascii_run(&mut self, text: &[u8]) {
        ...
        while offset < text.len() {
            ...
            let take = (text.len() - offset).min(columns - column);
            let has_wide = self.grid[line][Column(column)..Column(column + take)]
                .iter()
                .any(|cell| {
            ...
            for (cell, &byte) in self.grid[line][Column(column)..Column(column + take)]
                .iter_mut()
                .zip(&text[offset..offset + take])
            {
            ...
            self.damage
                .damage_line(line.0 as usize, column, column + take - 1);
            offset += take;
            if column + take < columns {
                self.grid.cursor.point.column = Column(column + take);
            } else {
