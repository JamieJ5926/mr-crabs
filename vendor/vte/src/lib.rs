...
pub trait Perform {
    ...
    /// Draw a run of ground-state text. Implementors can override this to
    /// amortize handler dispatch; the default preserves byte-for-byte actions.
    fn print_run(&mut self, text: &str) {
        for c in text.chars() {
            match c {
                '\x00'..='\x1f' | '\u{80}'..='\u{9f}' => self.execute(c as u8),
                _ => self.print(c),
            }
        }
    }
